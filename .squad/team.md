# Squad Team

> github-copilot-101 — hands-on 90-min labs for learning GitHub Copilot

## Coordinator

| Name | Role | Notes |
|------|------|-------|
| Squad | Coordinator | Routes work, enforces handoffs and reviewer gates. |

## Members

| Name | Role | Charter | Status |
|------|------|---------|--------|
| Zoe | Lead | `.squad/agents/zoe/charter.md` | ✅ Active |
| Book | Copilot Expert (Platform, Surfaces, Patterns & Prompting) | `.squad/agents/book/charter.md` | ✅ Active |
| Kaylee | Lab Builder | `.squad/agents/kaylee/charter.md` | ✅ Active |
| Jayne | Tester | `.squad/agents/jayne/charter.md` | ✅ Active |
| Simon | Learner | `.squad/agents/simon/charter.md` | ✅ Active |
| Scribe | Session Logger | `.squad/agents/scribe/charter.md` | 📋 Silent |
| Ralph | Work Monitor | — | 🔄 Monitor |

## Project Context

- **Owner:** Geronimo Basso
- **Project:** github-copilot-101
- **Description:** Hands-on labs (90 min, theory + practice) teaching GitHub Copilot end-to-end. Existing examples: `customize-copilot/`, `copilot-chat/`.
- **Stack:** Markdown labs, multi-language sample code
- **Created:** 2026-04-20

